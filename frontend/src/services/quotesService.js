...
async function calculateSnapshots(modelId, fabricId, variantIds, attributeIds, clientTypeId) {
    // Busca o modelo
    ...
    if (!clientType)
        throw new Error(`Tipo de cliente não encontrado: ${clientTypeId}`);
...
export async function createQuote(items) {
    if (items.length === 0)
    ...
    for (const item of items) {
        const snapshots = await calculateSnapshots(item.modelId, item.fabricId, item.variantIds, item.attributeIds, item.clientTypeId);
        // Cria o quote_item
