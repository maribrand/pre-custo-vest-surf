...
async function mapFromDb(row: any): Promise<ProductModel> {
  let categoryName = '';
  if (row.category_id) {
...
async function mapToDb(data: Omit<ProductModel, 'id'> | ProductModel): Promise<any> {
...
export async function getAllProductModels(): Promise<ProductModel[]> {
...
export async function createProductModel(data: Omit<ProductModel, 'id'>): Promise<ProductModel> {
...
export async function updateProductModel(id: string, data: Partial<Omit<ProductModel, 'id'>>): Promise<ProductModel> {
  const updateData: any = {};
  
  if (data.name !== undefined) updateData.name = data.name;
  if (data.baseCost !== undefined) updateData.base_cost = data.baseCost;
  if (data.fabricConsumption !== undefined) updateData.base_fabric_consumption_m = data.fabricConsumption;
