...
async function mapFromDb(row: any, modelId?: string): Promise<Fabric> {
...
export async function getFabricsByModel(modelId: string): Promise<Fabric[]> {
  ...
  const mappedFabrics = await Promise.all(
...
export async function createFabric(
  ...
  return getFabricsByModel(modelId).then(fabrics => 
...
export async function updateFabric(
  id: string,
  data: Partial<Omit<Fabric, 'id' | 'modelId' | 'totalCost'>>
): Promise<Fabric> {
  const updateData: any = {};
  if (data.name !== undefined) updateData.name = data.name;
  if (data.unitCost !== undefined) updateData.cost_per_meter = data.unitCost;
...
export async function deleteFabricFromModel(fabricId: string, modelId: string): Promise<void> {
